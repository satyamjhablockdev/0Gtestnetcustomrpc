# 0G Custom RPC guide
Useful for 0G storage node
**bash manager** that installs and runs a 0G Galileo testnet RPC (geth + 0gchaind), wraps it with **Nginx + TLS**, and lets you **grant access (IP allowlist + optional Basic Auth)**, **see logs**, **update**, **health-check**, etc.

> Works on Ubuntu 20.04+/22.04+. Run as root (or with `sudo -E`).  
> Default ports: 8545 (HTTP), 8546 (WS), 8551 (authrpc), 26656/26657 (p2p/Tendermint), 3500 (node API).  
> Customize via the ENV block at the top.
...
### How to use
1) Save as `0g-rpc-manager.sh`, make it executable:
```bash
chmod +x 0g-rpc-manager.sh
```

2) Run install (replace domain and optionally the release):
```bash
DOMAIN=rpc.yourdomain.com ENABLE_NGINX=true USE_LETSENCRYPT=true ./0g-rpc-manager.sh install
```

3) Grant access (pick **either** IP allowlist or Basic Auth—or both):
```bash
# Allow an office IP:
./0g-rpc-manager.sh access add-ip 203.0.113.10

# Create a Basic Auth account (will prompt password):
./0g-rpc-manager.sh access add-user alice
```

4) Check logs & health:
```bash
./0g-rpc-manager.sh logs follow
./0g-rpc-manager.sh health
./0g-rpc-manager.sh rpc test-public
```

5) Routine ops:
```bash
./0g-rpc-manager.sh upgrade        # replace binaries with OG_RELEASE
./0g-rpc-manager.sh backup         # snapshot data quickly
./0g-rpc-manager.sh restore /root/0g-backup-YYYYMMDD-HHMM
./0g-rpc-manager.sh firewall lockdown
```
